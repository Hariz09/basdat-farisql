  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-6 pt-32 pb-16">
        ...
        <h1
          className="font-serif font-black leading-[0.88] tracking-tight mb-3"
          style={{ fontSize: "clamp(80px, 13vw, 140px)" }}
        >
          Tik
        ...
        <p className="text-sm text-black/50 max-w-xs leading-relaxed mb-10">
          Proyek basis data untuk pengelolaan acara — tugas mata kuliah Basis
          Data B, Fasilkom UI.
        </p>

        <Button
          asChild
          className="rounded-md bg-black text-white hover:bg-black/80 px-6"
        >
          <Link href="/login">
            Masuk ke Sistem <ArrowRight size={14} className="ml-2" />
          </Link>
        </Button>
      </main>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 border border-black/10 rounded-2xl overflow-hidden max-w-3xl w-full">
          {teamMembers.map((m, i) => (
            ...
            >
              ...
              <p className="font-semibold text-xs leading-snug text-black">
                {m.name}
              </p>
            </div>
          ))}
